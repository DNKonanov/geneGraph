...
def greedy_collapse(graph):
    ...
    for genome in graph:
        for contig in graph[genome]:
            ...
            for i in range(1, len(chain)-1):
                ...
                if not node.startswith('PG'):
                    continue

                
                left = int(chain[i-1].split('_')[0][2:])
                right = int(chain[i+1].split('_')[0][2:])
    ...
    for gene in context_map:

        master_gene = int(gene.split('_')[0][2:])
        
        if master_gene not in master_map:
            master_map[master_gene] = {}
        
        master_map[master_gene][gene] = context_map[gene]


    gene_replaces = {}
    for master_gene in master_map:

        renamed = set()
        for gene in master_map[master_gene]:

            if gene in renamed:
                continue

            context = master_map[master_gene][gene]

            for other_gene in master_map[master_gene]:
                if other_gene in renamed:
                    continue
                    
                other_context = master_map[master_gene][other_gene]

                if len(other_context.intersection(context)) > 0:
                    gene_replaces[other_gene] = gene

                    renamed.add(other_gene)

    return gene_replaces
...
for genome in coord_table:
    new_coord_table[genome] = {}

    for c in coord_table[genome]:
